...
const category = 'business';  // Updated category to business
...
// Top News Fetcher for business
export async function fetch_top_news() {
    let stored_data = await getDataFromFirestore('Business', 'top_news');  // Updated collection to 'business'
    if (stored_data && stored_data.articles) {
        ...
        return await fetchAndStoreNews(url, 'Business', 'top_news');  // Store in 'business' collection
    }
}

// Latest News Fetcher for business
export async function fetch_latest_news() {
    let stored_data = await getDataFromFirestore('Business', 'latest_news');  // Updated collection to 'business'
    if (stored_data && stored_data.articles) {
        return stored_data.articles;
    } else {
        const url = `https://newsapi.org/v2/everything?q=business&from=2024-09-27&to=2024-09-27&sortBy=publishedAt&apiKey=${apikey}`;
        try {
            const response = await fetch(url);
            if (!response.ok) {
            ...
            // Store in Firestore in 'business' collection
            await storeDataInFirestore('Business', 'latest_news', { articles: data.articles });
            return data.articles;
        } catch (error) {
...
// Popular News Fetcher for business
export async function fetch_popular_news() {
    let stored_data = await getDataFromFirestore('Business', 'popular_news');  // Updated collection to 'business'
    if (stored_data && stored_data.articles) {
        return stored_data.articles;
    } else {
        const url = `https://newsapi.org/v2/everything?q=business&from=2024-09-27&to=2024-09-27&sortBy=popularity&apiKey=${apikey}`;
        return await fetchAndStoreNews(url, 'Business', 'popular_news');  // Store in 'business' collection
    }
}

// Related News Fetcher for business
export async function fetch_related_news() {
    let stored_data = await getDataFromFirestore('Business', 'related_news');  // Updated collection to 'business'
    if (stored_data && stored_data.articles) {
        return stored_data.articles;
    } else {
        const url = `https://newsapi.org/v2/everything?q=business&from=2024-09-27&to=2024-09-27&sortBy=relevancy&apiKey=${apikey}`;
